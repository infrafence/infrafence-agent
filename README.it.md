...
- **Liste di threat intelligence** — scaricate da ogni server direttamente dagli editori una volta al giorno: reti note come dirottate, la lista di host compromessi di Emerging Threats e le impronte degli user agent dei bot ([crawler-user-agents](https://github.com/monperrus/crawler-user-agents), [ai.robots.txt](https://github.com/ai-robots-txt/ai.robots.txt), entrambe MIT). Richiede HTTPS in uscita verso gli editori (URL in [`internal/intel/intel.go`](internal/intel/intel.go)). Il blocco in entrata delle reti elencate richiede `ipset`; senza, le liste servono solo al rilevamento egress/DNS.
