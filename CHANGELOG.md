...
- **feat: dashboard changes reach servers in real time.** Settings, whitelist, WAF rules, bans, finding actions, "Scan now" and YARA installs used to wait for the next 5-minute sync. Each agent now subscribes to its own Supabase Realtime channel and syncs as soon as the dashboard signals a change (~0.3 s). If that channel is down, a config version in the heartbeat catches it within 60 s, and the 5-minute sync remains the safety net. A push is only a "sync now" hint: the agent never acts on its contents and always fetches its configuration through the authenticated API. All syncs now run one at a time (a websocket-requested sync could previously overlap the periodic one), bursts are merged, at most one sync every 3 s.
- **feat: all firewall enforcement lives in dedicated `INFRAFENCE` chains, and repairs itself.** Bans, threat-feed and country blocks, and dashboard rules no longer insert rules into `INPUT`: one marked jump from `INPUT` (and `DOCKER-USER`, so published container ports are covered) leads to InfraFence's own chains, rebuilt atomically with `iptables-restore --noflush`. The jump is placed before the first rule that can accept traffic, so ufw/firewalld `ACCEPT`s can't bypass a ban. Whitelisted and protected IPs are exempted with `RETURN`, never `ACCEPT`, so InfraFence never opens anything. A guard checks every 60 s and restores the chains and jumps if a firewall reload (ufw, firewalld, CSF, iptables-restore) removed them (`firewall_rules_restored`, `firewall_conflict` when it keeps happening). Rules from older versions are migrated out of `INPUT` only when provably the agent's. Verified with real traffic on nf_tables and legacy backends, with and without ipset.
- **feat: preflight host scan.** `infrafence-agent preflight [--json]` inspects the server read-only: iptables backend, ipset, ufw/firewalld/CSF/fail2ban/CrowdSec/Docker/Kubernetes, web servers, hosting panels, configuration management, running package operations, disk and memory. The installer runs it before changing anything; the agent at startup and daily, reported as `host_preflight` (shown on the dashboard's host compatibility card).
- **feat: conservative defaults for host changes, set from the dashboard.** Web server configuration changes (UA blocking in nginx/Apache, ModSecurity rules) are now **off by default** and never made where a hosting panel or configuration management owns the config; previously every nginx server block was edited at first start. Bots set to "block" are still banned at the firewall. Inbound blocking from threat-intelligence lists is **off by default** (the lists still drive egress/DNS detection). Auto-update can be set to notify-only (`update_available`). Changes made by earlier versions are not removed automatically.
- **feat: LiteSpeed support.** LiteSpeed Enterprise and OpenLiteSpeed are detected (they live outside PATH) and reported as the web server. LiteSpeed Enterprise keeps Apache's `httpd` for its configuration, so the agent used to believe Apache served the sites and could edit and reload Apache under LiteSpeed; while LiteSpeed serves the sites, InfraFence no longer edits any web server configuration. OpenLiteSpeed and CyberPanel per-site access logs are now found from their configuration.
- **feat: `install.sh --dry-run` and a clean uninstall.** The dry run shows the preflight and what would be done, and changes nothing. The installer installs ipset only when preflight says it's safe (`--no-ipset` to skip), waits for running package operations, no longer runs `dpkg --configure -a` or `apt-get --fix-broken` (they can restart unrelated services), and no longer modifies the system CA bundle. `install.sh --uninstall` now runs `infrafence-agent uninstall --clean`, which removes exactly what InfraFence added: firewall chains, jumps and sets, UA blocking, ModSecurity includes (restored if the web server's config test fails).
- **fix: a failed ModSecurity setup could break the next Apache restart.** A failed `apachectl configtest` left the added include on disk. Setup is now transactional.
